...
Open **More > Import map blueprint** and drop a PNG, JPG, or WebP top-down plan, or select all six standardized bundle sheets at once. Set the measured plan width, choose trace detail, then review the highlighted geometry before selecting **Create editable map**. On clean technical boards, the largest connected light/colored floor becomes the authoritative playable mask; letter-sized marks are removed while courtyards and interior wall islands are preserved. Orange and blue regions anchor the two teams, red and yellow anchor A/B, green regions become safe-clearance cover, and purple/orange bands describe lower/elevated areas. The bundle importer aligns the map panels in the blueprint, route, and elevation sheets, merges corroborated evidence, and converts repeated stair, ramp, ladder, platform, and cover cues into editable compile-safe structures with landings. Older dark-wall plans continue to use the legacy enclosed-area fallback.

The generated material kit samples a single plan palette or cuts a supplied material sheet into 20 categorized 256×256 roles. If the companion is connected, they are atomically installed into Blockout's custom WAD. The semantic assignment pass then textures every generated room surface, traced wall, door, crate, stair, ramp, ladder, platform, and generated prefab with the appropriate wall, floor, ceiling, trim, wood, metal, or accent family. Unneeded variants remain available in the categorized material selectors. Browser-only use falls back to categorized included materials, so map creation never depends on a cloud service or a secret API key.
